...
func initializeStateMachine(state *machine.StateMachine, t *testing.T) {
	state.StartAt = to.Strp("Start")
	state.States = machine.States{}
...
func Test_MapState_ValidateResource(t *testing.T) {
	...
	state.Iterator = &machine.StateMachine{}
	assert.Error(t, state.Validate())
	initializeStateMachine(state.Iterator, t)
...
func Test_MapState_Integration(t *testing.T) {
	...
	// Default
	var task = state.Iterator.States["Task"].(*machine.TaskState)
